...
import { APP_CONFIG, isServiceEnabled } from "@flusys/ng-core";
  ...
  return sessionRestore$.pipe(
    switchMap((initSuccess): Observable<boolean | UrlTree> => {
      ...
      if (!initSuccess || !authState.user() || !authState.accessToken()) {
        if (isDevMode())
          console.warn("[AppInitGuard] Auth failed, redirecting to login");
        return of(loginUrl);
      }

      // Check company/branch selection (if feature enabled)
      if (
        appConfig.enableCompanyFeature &&
        (!authState.company() || !authState.branch())
      ) {
        if (isDevMode())
          console.warn("[AppInitGuard] Missing company/branch selection");
        return of(companyUrl);
      }

      // Load IAM permissions (if feature enabled and not already loaded)
      if (isServiceEnabled(appConfig, "iam") && !permissionState.isLoaded()) {
