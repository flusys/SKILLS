import { appSwaggerConfig } from "@/modules/shared/swagger.config";
import { AiAssistantModule } from "@flusys/nestjs-ai-assistant";
import { aiAssistantSwaggerConfig } from "@flusys/nestjs-ai-assistant/docs";
...
import { taskManagerSwaggerConfig } from "@flusys/nestjs-task-manager/docs";
...
export function configureSwaggerDocs(
  ...
  const permissionMode = PermissionModeHelper.fromString(
    bootstrapAppConfig.permissionMode,
  );
  const { enableCompanyFeature } = bootstrapAppConfig;

  const swaggerConfigs: IModuleSwaggerOptions[] = [
    {
      ...authSwaggerConfig(bootstrapAppConfig),
      modules: [AuthModule],
      globalHeaders,
    },
    {
      ...iamSwaggerConfig(
        ...
        permissionMode,
        bootstrapAppConfig.databaseMode,
      ),
      modules: [IAMModule],
      globalHeaders,
    },
    {
      ...storageSwaggerConfig(bootstrapAppConfig),
      modules: [StorageModule],
      globalHeaders,
    },
    {
      ...formBuilderSwaggerConfig(bootstrapAppConfig),
      modules: [FormBuilderModule],
      globalHeaders,
    },
    {
      ...emailSwaggerConfig(bootstrapAppConfig),
      modules: [EmailModule],
      globalHeaders,
    },
    {
      ...eventManagerSwaggerConfig(bootstrapAppConfig),
      modules: [EventManagerModule],
      globalHeaders,
    },
    {
      ...notificationSwaggerConfig(bootstrapAppConfig),
      modules: [NotificationModule],
      globalHeaders,
    },
    {
      ...localizationSwaggerConfig(bootstrapAppConfig),
      modules: [LocalizationModule],
      globalHeaders,
    },
    {
      ...taskManagerSwaggerConfig(),
      modules: [TaskManagerModule],
      globalHeaders,
    },
    {
      ...aiAssistantSwaggerConfig(bootstrapAppConfig),
      modules: [AiAssistantModule],
      globalHeaders,
    },
    {
      ...appSwaggerConfig(),
      modules: [
        //All business modules here, so that we can have a single swagger doc for all business modules
      ],
      globalHeaders,
    },
  ];
  ...
  logger.log(
    `Storage API docs:       http://localhost:${port}/api/docs/storage`,
  );
  logger.log(
    `Form Builder API docs:  http://localhost:${port}/api/docs/form-builder`,
  );
  logger.log(`Email API docs:         http://localhost:${port}/api/docs/email`);
  logger.log(
    `Event Manager API docs: http://localhost:${port}/api/docs/event-manager`,
  );
  logger.log(
    `Notification API docs:  http://localhost:${port}/api/docs/notification`,
  );
  logger.log(
    `Localization API docs:  http://localhost:${port}/api/docs/localization`,
  );
  logger.log(
    `Task Manager API docs:  http://localhost:${port}/api/docs/task-manager`,
  );
  logger.log(
    `AI Assistant API docs:  http://localhost:${port}/api/docs/ai-assistant`,
  );
  logger.log(`AI Assistant API docs:  http://localhost:${port}/api/docs/app`);
}
