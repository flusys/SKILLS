...
const ENABLE_STORAGE = true; // Set false if not using Storage module
const ENABLE_FORM_BUILDER = true; // Set false if not using Form Builder module
const ENABLE_EMAIL = true; // Set false if not using Email module
const ENABLE_EVENT_MANAGER = true; // Set false if not using Event Manager module
const ENABLE_LOCALIZATION = true; // Set false if not using Localization module
const ENABLE_NOTIFICATION = true; // Set false if not using Notification module
const ENABLE_TASK_MANAGER = true; // Set false if not using Task Manager module
// ============================================================================
...
function buildActionTree(): ActionDefinition[] {
  ...
  return [
    ...
    ...(ENABLE_LOCALIZATION
      ...
      : []),
    // Task Manager Module (conditional)
    ...(ENABLE_TASK_MANAGER
      ? [
          {
...
function getAllPermissionCodes(): string[] {
  ...
  if (ENABLE_TASK_MANAGER) {
    codes.push(...Object.values(TASK_BOARD_PERMISSIONS));
    codes.push(...Object.values(TASK_LIST_PERMISSIONS));
    codes.push(...Object.values(TASK_PERMISSIONS));
    codes.push(...Object.values(TASK_LABEL_PERMISSIONS));
    codes.push(...Object.values(TASK_COMMENT_PERMISSIONS));
  }
...
async function seedAdmin(): Promise<void> {
  ...
  console.log(`   - enableTaskManager: ${ENABLE_TASK_MANAGER}`);
  console.log('');
  ...
  try {
    ...
    let user: UserRow | undefined = (
      await queryRunner.query(
        buildQuery(
          `SELECT id, email, name FROM app_user WHERE email = ? AND deleted_at IS NULL LIMIT 1`,
          dbType,
        ),
        [adminEmail],
      )
