...
const ENABLE_EMAIL = bootstrapAppConfig.enableEmailVerification ?? true;
// ============================================================================
  ...
  const createCrudActions = (
    ...
    return Object.entries(permissions).map(([key, code]) => {
      ...
      return {
        ...
        code,
        description: `${label} ${moduleName.toLowerCase()}`,
    ...
    for (const action of allActions) {
      ...
      if (!existingPermission) {
        ...
        if (ENABLE_COMPANY_FEATURE && company) {
          await queryRunner.query(
            buildQuery(
              `INSERT INTO ${permissionTable} (id, permission_type, source_type, source_id, target_type, target_id, company_id)
               VALUES (?, ?, ?, ?, ?, ?, ?)`,
              dbType,
            ),
            [
          ...
          await queryRunner.query(
            buildQuery(
              `INSERT INTO ${permissionTable} (id, permission_type, source_type, source_id, target_type, target_id)
               VALUES (?, ?, ?, ?, ?, ?)`,
              dbType,
            ),
            [
