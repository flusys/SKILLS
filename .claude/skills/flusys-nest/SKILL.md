...
**Import order in AppModule:** `NotificationModule` before `AuthModule` so `NOTIFICATION_ADAPTER` token is available. If notification features are enabled.
...
### `ApiService` — CRUD service base class

`ApiService` is the base for all entity services. Import from `@flusys/nestjs-shared/classes`.
...
export class MyService extends ApiService<CreateDto, UpdateDto, IModel, Entity> {
  constructor(
    private readonly dataSourceProvider: MyDataSourceProvider,
    @Inject(UtilsService) protected readonly utilsService: UtilsService,
  ) {
    //         entityName      cache                utilsService  serviceName      cache?  slug   EntityClass  provider
    super("myentity", new HybridCache(60000), utilsService, MyService.name, true, "APP", MyEntity, dataSourceProvider);
  }
}
```

| super() arg | Type | Example |
| --- | --- | --- |
| entityName | `string` | `"customer"` — QueryBuilder alias |
| cache | `HybridCache` | `new HybridCache(60000)` — TTL in ms |
| utilsService | `UtilsService` | injected via `@Inject(UtilsService)` |
| serviceName | `string` | `MyService.name` |
| isCacheable | `boolean` | `true` |
| moduleName | `string` | `"ISP"` — app/module slug |
| EntityClass | entity class | `MyEntity` |
| dataSourceProvider | provider | injected directly |

#### Repository access

```typescript
// Primary entity — initialize first, then use this.repository
await this.ensureDataSourceRepository();
const item = await this.repository.findOne({ where: { id } });
await this.repository.save(entity);

// Cross-entity (other entities) — get via provider
...
#### Override hooks — implement only what you need

```typescript
// DTO → Entity mapping (always call super first)
...
protected override async getSelectQuery(
  ...
  query.addSelect(["myentity.createdAt", "myentity.updatedAt"]);
...
protected override async getFilterQuery(
  ...
  if (filter.status) query.andWhere("myentity.status = :status", { status: filter.status });
  return { query, isRaw: false };
}
...
protected override async getExtraManipulateQuery(
  ...
  query.leftJoinAndSelect("myentity.relation", "relation");
  query.andWhere("myentity.companyId = :companyId", { companyId: user.companyId });
  query.andWhere("myentity.branchId = :branchId", { branchId: user.branchId });
  return { query, isRaw: false };
}

// Pre-insert side effects (runs inside transaction)
protected override async afterInsertOperation(
  ...
  for (const entity of entities) {
    await queryRunner.manager.save(OtherEntity, { entityId: entity.id });
  }
}

// Pre-delete cleanup (runs inside transaction)
...
### `createApiController` — CRUD controller factory

Import from `@flusys/nestjs-shared/classes`. **Always use the `const BaseController` pattern — never extend inline on the class.**
...
const BaseController = createApiController(
  ...
  MyEntityResponseDto,
  {
    entityName: "myentity",      // QueryBuilder alias, drives messageKey prefixes
    security: "jwt",             // uniform JWT for all endpoints
    enabledEndpoints: [          // omit this array to enable all 10 endpoints
...
export class MyEntityController extends BaseController {
  constructor(private readonly myEntityService: MyEntityService) {
    super(myEntityService);
  }

  // Add extra endpoints inside the class body when needed:
  @Patch("activate")
  @UseGuards(JwtAuthGuard)
  @ApiOperation({ summary: "Activate entity" })
  @ApiResponseDto(MyEntityResponseDto)
  async activate(
    @Body() dto: ActivateDto,
    @CurrentUser() user: ILoggedUserInfo,
  ) {
    const data = await this.myEntityService.activate(dto, user);
    return { success: true, message: "Activated", data };
  }
}
```

**Exposed endpoints (all POST):**
...
| `getByIds` | `POST /get-by-ids` | Fetch array |
| `getAll` | `POST /get-all` | Paginated list |
| `getByFilter` | `POST /get-by-filter` | First match |
| `bulkUpsert` | `POST /bulk-upsert` | Upsert array |
| `update` | `POST /update` | Update one |
| `updateMany` | `POST /update-many` | Update array |
| `delete` | `POST /delete` | Soft / restore / permanent |

**Security options for `createApiController`:**

```typescript
// Option A — uniform JWT (most common)
{ security: "jwt", entityName: "..." }

// Option B — uniform permission
{ security: { level: "permission", permissions: ["entity.read"] }, entityName: "..." }

// Option C — partial CRUD (disable endpoints not needed)
{ security: "jwt", entityName: "...", enabledEndpoints: ["insert", "getAll", "getById", "update", "delete"] }
```
