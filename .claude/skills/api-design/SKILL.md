...
Do **not** extend `ApiService` — that is CRUD-only.
...
- `resolveEntity`, `getFilterQuery`, or any CRUD hook methods — those belong to `ApiService` only
...
| [flusys-nest](../flusys-nest/SKILL.md)                     | `ApiService`, `createApiController`, adapter tokens, all response DTOs                     |
| [engineering](../engineering/SKILL.md)                     | Entity base class, migrations, soft delete, N+1 prevention                                 |
