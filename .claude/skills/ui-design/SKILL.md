...
| Component | Selector | When to use |
| --- | --- | --- |
| `TableComponent` | `f-table` | **The** list-page component. Always `[lazy]="true"` bound to a backend-paginated service — never load everything client-side for lists that can grow. Its header/body/emptymessage/caption/footer/loadingbody/paginatorleft/paginatorright/rowexpansion slots are template reference variables (`<ng-template #header>`, `<ng-template #body let-item>`, `<ng-template #emptymessage>`) — **not** `fTemplate="header"`. `f-table` never declares `contentChildren(FTemplateDirective)`, so `fTemplate="..."` on its content is silently inert: no error, no console warning, the `<ng-template>` just never instantiates, so the header renders no `<th>`, the body renders no `<tr>`, and the page looks like a data-loading failure even when the underlying `get-all` call returns 200 with real rows. `f-dialog`'s `header`/`footer` slots work the same way (`#header`/`#footer`, not `fTemplate`). Before trusting `fTemplate="x"` on any component, grep its compiled `queries:` array in `node_modules/@flusys/ng-ui` — a `predicate: ["x"]` array of bare strings means a `#x` ref var is required instead. |
...
| — | — | **Any `f-select`, `f-multiselect`, `f-autocomplete`, `f-treeselect`, `f-cascadeselect`, `f-menu`, or `f-password` placed inside an `f-dialog`/`f-drawer`, a `<table>`/`f-table` row, or any other scrollable or `overflow:hidden`/`overflow:auto` container needs `[appendTo]="'body'"`.** These seven default `appendTo` to `'self'` — their overlay panel renders inline inside that container's own DOM subtree, so it gets clipped at the container's edge instead of floating above it. This is not just a dialog problem: a `<td>` holding an inline-edit `f-select`, or any field near the bottom of a scrolling panel/sidebar, clips the same way — a truncated or invisible option list, not an error, so it's easy to miss in a quick check. `f-datepicker`, `f-colorpicker`, `f-popover`, and `f-splitbutton` already default to `'body'` and need no override. Rule of thumb: if the component sits anywhere other than directly in the page's normal flow — a table row, a dialog/drawer body, a scrollable panel — add `[appendTo]="'body'"` by default rather than waiting to notice the clipping. |
...
| `FTemplateDirective` | `ng-template[fTemplate]` | Named slots on components that actually declare `contentChildren(FTemplateDirective)` (e.g. `f-image`'s `indicator`, `f-dataview`'s `list`/`grid`). **Not universal** — verify per component before using it, see the `f-table` gotcha below |
...
- **Every filter row's wrapper `<div>` carries a `filter-bar` class, and no filter control needs its own width binding.** At least 20 of `@flusys/ng-ui`'s ~48 host-classed components — `f-select`, `f-datepicker`, `f-multiselect`, `f-autocomplete`, `f-treeselect`, `f-cascadeselect`, `f-iftalabel`, and others — compile `hostClass` to `block w-full ${styleClass()}` unconditionally, correct for a form field filling its `flex-col` wrapper but wrong the moment two or more sit in one `flex flex-wrap` filter row: each stretches to 100% and they stack instead of sitting side by side. Rather than remembering a per-instance `[style]="{ width: '...' }"` on every filter control forever (easy to do for `f-select`, then miss on `f-datepicker`, then miss again on the next component someone reaches for — this happened twice in this project before the structural fix), `dashboard/src/tailwind.css` has one unlayered CSS rule, `.filter-bar > .w-full { width: auto; min-width: 180px; max-width: 240px; }`, that overrides the `w-full` utility class for any direct child of a `filter-bar`-marked row — permanently, for every current filter control and any future one the library adds with the same default, no JS binding required. It works because Tailwind wraps its generated utilities in `@layer utilities`, and per CSS Cascade Layers, unlayered author CSS always beats layered CSS regardless of selector specificity or source order. Every generated filter row's markup is `<div class="filter-bar flex flex-wrap items-center gap-2 mb-4">` — keep that exact class on any new one; don't add `[style]` width bindings to its children, the CSS rule already covers them.
...
- A `computed()` reading `formControl.value` directly instead of a plain writable `signal()` — `FormControl.value` is a getter, not a signal, so `computed()` never re-tracks it. A binding like `[disabled]="!computedThing()"` silently stops updating as the user types, with no error, and is especially easy to miss in a zoneless app where nothing else forces a re-render. Back any computed that must depend on form-like input with a `signal()` instead.
- Converting a date-only value to a wire string with `date.toISOString().slice(0, 10)`. `toISOString()` converts to UTC first, shifting the calendar date by a day for any viewer with a negative UTC offset — picking "2026-01-01" at local midnight can serialize as "2025-12-31". Build the string from the `Date`'s local Y/M/D parts instead. On the entity side, a `date`-only (not `datetime`) column's TS property should be typed `string`, not `Date`, so the value never round-trips through a `Date` object on the backend either.
- A backtick anywhere inside a standalone component's inline `template: \`...\`` string — including inside an HTML `<!-- -->` comment, e.g. a markdown-style code-span like `` `some-selector` `` in a doc-comment. The surrounding `template:` value is itself a JS template literal; a bare backtick prematurely closes it, and the compiler then tries to parse the rest of the file as code — producing a cascade of unrelated, misleading errors (`NG1002 Incorrect number of arguments to @Component decorator`, `Unterminated regular expression`, `Cannot find name X`) with nothing pointing at the real backtick. Write such comments without backticks, or move a large template to a `templateUrl` file instead.
