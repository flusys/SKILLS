...
| — | — | **Any `f-select`, `f-multiselect`, `f-autocomplete`, `f-treeselect`, `f-cascadeselect`, `f-menu`, or `f-password` placed inside an `f-dialog`/`f-drawer` needs `[appendTo]="'body'"`.** These seven default `appendTo` to `'self'` — their overlay panel renders inline inside the dialog's own DOM subtree, so it gets clipped at the dialog's edge instead of floating above it (a `f-select` two rows from the bottom of a short dialog shows a truncated or invisible option list, not an error). `f-datepicker`, `f-colorpicker`, `f-popover`, and `f-splitbutton` already default to `'body'`. This applies outside dialogs too, anywhere a clipping/scrolling ancestor (`overflow:hidden`/`auto`) sits between the field and the viewport — not just `f-dialog`. |
