...
| Skill             | When loaded                           |
| ----------------- | ------------------------------------- |
| `engineering`  | Step 3 — DB design, Step 7 — quality, Step 8 — caching, Step 9 — security |
...
Load skill: **engineering** (Database Design section) — follow all its rules for column types, relations, indexes, and migrations.
...
| 4 | `<module>.service.ts` | Extends `ApiService`, injects project DataSource provider |
...
- Add menu entry to `config/app-menu.config.ts` — use `label` (hardcoded English string) if `ng-localization` is not selected; use `labelKey` (translation key) if it is; always include `icon` and `routerLink`

---

### Step 7 — Code Quality Review

Load skill: **engineering** (Code Quality section) — apply to every file generated in Steps 3–6. Fix all issues before proceeding.
...
Load skill: **engineering** (Performance & Caching section) — apply only where the PRD or data volume warrants; do not cache speculatively.
...
Load skill: **engineering** (Security section) — audit every generated file. Fix all critical findings before Step 10.
