...
| `domain-events.md` | `backend/src/consumers/*.ts`, `backend/src/modules/**/consumers/*.ts`, `backend/src/config/modules.config.ts` | Events are fire-and-forget — nothing awaits a handler, errors are swallowed, delivery is dropped during a broker outage; consumers must be idempotent singletons, and `moduleEventsConfig`'s `actions`/`entities` allowlists decide what ever reaches them |
