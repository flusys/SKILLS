...
2. `docs/prd-feature-<nn>-<name>.md` — one file per feature module, consumed by `/develop-feature` (`nn` = two-digit order index, e.g. `01`, `02`)
...
> **Module serialization rule:** Feature PRD files must be ordered so that modules with no dependencies come first, and modules that depend on others come after. A dependent module cannot be developed before its base module is complete — `/develop-feature` runs one file at a time in sequence. Always reflect this order in both the `Feature Modules (development order)` section of `prd-bootstrap.md` and the output summary in Step 6.
...
| --------------------------------------- | -------- | -------------------------- |
| nestjs-auth / ng-auth                   | yes/no   | <signal from requirements> |
...
**Always included:** nestjs-core, nestjs-shared / ng-core, ng-shared, ng-layout
...
1. `docs/prd-feature-01-<name1>.md` — no dependencies
2. `docs/prd-feature-02-<name2>.md` — depends on <name1>
3. ...
```

---

## Step 4 — Write `docs/prd-feature-<nn>-<name>.md` (one file per module)

Name files using a two-digit order prefix that matches the development order: `prd-feature-01-invoice.md`, `prd-feature-02-invoice-item.md`, `prd-feature-03-user-management.md`, etc. Assign `nn` after resolving inter-module dependencies in Step 2.2.
...
Feature PRDs:
  - docs/prd-feature-01-<name1>.md   (<EntityCount> entities, <EndpointCount> endpoints)
  - docs/prd-feature-02-<name2>.md   (<EntityCount> entities, <EndpointCount> endpoints)
...
TODO items remaining (require manual input before running commands):
  ...
  - docs/prd-feature-<nn>-<name>.md: <N> items — <list fields that are TODO>

Consistency check: PASSED | FIXED (<what was fixed>)

Development order:
  1. /bootstrap docs/prd-bootstrap.md
  2. /develop-feature docs/prd-feature-01-<name1>.md
  3. /develop-feature docs/prd-feature-02-<name2>.md
