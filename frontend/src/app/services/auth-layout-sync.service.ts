...
import { FaviconService, FileUrlService } from '@flusys/ng-shared';
...
export class AuthLayoutSyncService {
  ...
  constructor() {
    ...
    effect(() => {
      this.layoutService.layoutConfig(); // track theme/color changes
      this.faviconService.apply();
    });
    ...
    toObservable(source)
      .pipe(
        switchMap((data): Observable<null> => {
          ...
          if (data && imageId && isServiceEnabled(this.appConfig, 'storage')) {
